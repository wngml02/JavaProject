...
public class suwon implements concerthall {
	...
	public void setWhere() {
		...
		hallGo = hallLocation.suwon; // 참조 열거타입 사용으로 콘서트 위치 정보 제공
		...
		if (where == 1) { // 레스토랑 선택 시
			...
			List<List<String>> list = ReadCsv.readCSVFile("src/csvfile/swlist.csv"); // 레스토랑 정보가 있는 csv 파일 받아오기
			...
			System.out.println(hallGo + " "+whereGo+"의 정보");  // 받아온 참조타입으로 정보 제시
			List<List<String>> list = ReadCsv.readCSVFile("src/csvfile/swcafe.csv"); //카페 정보가 있는 csv 파일 받아오기
		    for (List<String> line : list) {
		      System.out.println(String.join("||", line));
		    } // csv 정보 제공-List 컬렉션프레임워크 사용
		}
